...
from pyCCDA.documents import parse_address, parse_date
from .query_database import query_table_by_code_system
...
def encounters(ccda):
    ...
    for entry in encounters.entries():
        ...
        name = query_table_by_code_system(name, code, code_system, code_system_name)
        # translation
        ...
        for current in findings_els:
            ...
            if el.attr('code') == None and el.attr('displayName') == None:
                continue
            name=el.attr('displayName'),
            code=el.attr('code'),
            code_system=el.attr('codeSystem'),
            name = query_table_by_code_system(name, code, code_system, None)
            print("END: PRINT NAME HERE \n\n\n")
            print(name)
